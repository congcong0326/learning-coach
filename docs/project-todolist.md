...
| 当前主线任务 | T1：LLM 目标校准与学习计划生成 |
| 当前任务状态 | 未开始 |
| 已完成基础能力 | 全栈工程基座、题库 seed、题库表、题库 API、题库列表、工作台题面读取、本地注册登录、用户级 OpenAI API 资产池配置 |
| 下一步建议 | 开始 T1：用户填写自身情况后，由当前用户首选/当前通讯 OpenAI API 资产生成目标和任务草稿，用户确认后落库 |
| 第一版闭环状态 | 未闭环。当前已有题库、静态工作台、登录和 API 资产池基础，尚缺目标校准、学习计划、训练会话、AI 教练、代码运行、复盘画像和推荐 |
...
| 状态 | 未开始 |
| 前置条件 | T0、B1 已完成 |
| 主要交付 | 用户情况采集、LLM 目标和任务草稿生成、用户确认、学习计划页 |
...
- 用户可以填写目标、时间线、每周投入、首选语言和自评弱项。
- 后端使用当前用户首选/当前通讯 OpenAI API 资产生成目标和任务草稿。
- 用户可以确认或调整草稿，确认后目标和学习计划落库。
- 学习计划页能读取并展示当前用户计划。
...
- [ ] 设计 `user_learning_goal`、`study_plan`、`study_plan_item`。
- [ ] 实现目标校准输入 API。
- [ ] 使用当前用户首选/当前通讯 OpenAI API 资产生成目标和计划草稿。
- [ ] 实现用户确认草稿后落库。
- [ ] 实现学习计划读取、跳过和重排 API。
- [ ] 实现首访目标校准页。
- [ ] 实现学习计划页。
