...
| 项目 | 当前状态 |
| --- | --- |
| 当前阶段 | 阶段 1：学习入口与训练状态底座 |
| 当前主线任务 | T1：首访目标校准与学习计划基础 |
| 当前任务状态 | 未开始 |
| 已完成基础能力 | 全栈工程基座、题库 seed、题库表、题库 API、题库列表、工作台题面读取 |
| 下一步建议 | 先完成 T1，再做 T2；T1/T2 完成后进入基础 AI 教练和代码运行闭环 |
| 第一版闭环状态 | 未闭环。当前只有题库与静态工作台，尚缺目标校准、训练会话、AI 教练、代码运行、复盘画像和推荐 |
...
| 阶段 1 | 学习入口与训练状态底座 | 未开始 | 0% | T1、T2 | 用户可完成目标校准、生成学习计划，并在可恢复的训练会话中做题 |
| 阶段 2 | 基础反馈闭环 | 未开始 | 0% | T3、T5 | AI 可分层提示和 review，系统可运行 Python 代码并返回结构化结果 |
| 阶段 3 | Agent 状态机与知识增强 | 未开始 | 0% | T4、T6 | LangGraph 可恢复状态机接入，RAG 教练知识库可检索并受 hint level 控制 |
| 阶段 4 | 复盘画像与学习仪表盘 | 未开始 | 0% | T7 | 完成训练后生成复盘、画像增量、下一题推荐和仪表盘指标 |
| 阶段 5 | 面试模拟与可观测性 | 未开始 | 0% | T8、T9 | 支持轻量面试模拟，Trace 和 Eval 能展示 Agent 行为质量 |
| 阶段 6 | 第一版闭环集成 | 未开始 | 0% | T10 | PRD 第一版学习闭环可端到端跑通，构建和 smoke 验证通过 |

## 当前阶段：阶段 1

阶段 1 的目标是补齐“用户为什么练、接下来练什么、当前练到哪里”的业务底座。这个阶段完成前，不建议优先做复杂 AI、RAG 或面试模拟，因为后续能力都需要 session、event、plan 和 code snapshot 作为上下文。

### 当前任务 T1：首访目标校准与学习计划基础
...
| 状态 | 未开始 |
| 前置条件 | B0、B1 已完成 |
| 主要交付 | `user_learning_goal`、`study_plan`、`study_plan_item`、首访目标校准页、学习计划页 |
| 完成后解锁 | T2 训练会话与工作台状态持久化 |

**待办**

- [ ] 设计 `user_learning_goal`、`study_plan`、`study_plan_item` migration 和 SQLAlchemy model。
- [ ] 增加目标校准 API：创建/读取当前目标。
- [ ] 增加学习计划 API：生成当前计划、读取计划、调整顺序、跳过题目。
- [ ] 实现规则推荐：入门、面试冲刺、专项补弱、保持手感四类策略。
- [ ] 实现首访目标校准页。
- [ ] 实现学习计划页和推荐理由展示。
...
- 完成目标校准后 100% 生成基础训练计划。
- 每个推荐题目都有 `skill_tag`、推荐理由和建议训练模式。
- 计划页可以从推荐题进入工作台。
