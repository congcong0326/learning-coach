...
- 使用 Python 3.12 slim。
- 使用 uv 安装 Python 依赖。
- 安装 `ca-certificates`，满足 HTTPS 调用和基础证书需求。
- 运行 FastAPI 应用。
...
前端容器使用独立 volume 保存 `node_modules`，避免与宿主机或其他 WSL 前端项目互相影响。开发环境启动 frontend 服务时会先执行 `CI=true pnpm install --frozen-lockfile`，再启动 Vite dev server；这样当 `package.json` 或 `pnpm-lock.yaml` 变化时，容器内的依赖 volume 会以非交互方式同步。
