...
本文档是 T1 的早期规则版设计草案。新的产品决策是先实现 T0：本地用户注册登录与 OpenAI API 资产，然后把 T1 改为“用户填写自身情况 -> 使用当前用户默认 OpenAI API 资产生成目标和计划草稿 -> 用户确认 -> 落库”。

因此，本文档中关于 `local-user`、不实现登录、以及“不实现 AI 生成学习计划”的表述已经不再作为后续实现依据。T0 spec 完成后，需要重写或修订 T1 spec。
