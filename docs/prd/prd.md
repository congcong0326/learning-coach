...
1. 题库初始化与题目浏览。
2. 中文 Markdown 题面渲染。
3. 本地用户注册登录和用户级 OpenAI API 资产配置。
4. 首访目标校准：用户填写自身情况，由大模型生成目标和任务草稿，用户确认后落库。
5. 基础训练计划：根据用户确认后的目标、题库标签、难度和用户画像生成下一题或下一组训练建议。
6. 做题工作台：题面、Python 代码编辑器、AI 教练对话。
7. 两种核心训练模式：入门引导、独立训练。
8. 面试模拟模式的轻量版本：计时、追问、代码 review 和复盘评分，不做完整面试平台。
9. AI 教练分层提示，并在前端用用户可理解的提示档位展示。
10. 用户代码 review。
11. 本地代码运行工具，第一版支持 Python。
12. 用户手动回填 LeetCode 提交结果。
13. 单题复盘总结。
14. 用户学习画像更新。
15. 基础学习仪表盘：题型掌握度、常见卡点、平均提示等级和最近训练趋势。
16. Agent 执行 trace 记录。
17. 本地教程语料导入与 RAG 教练知识库接入。
...
登录页和注册页用于本地用户体系，不接第三方 OAuth。注册成功后自动创建后端 session，并进入 API 设置页。

API 设置页用于保存当前用户的 OpenAI API 资产：

- `display_name`：资产名称。
- `provider`：第一版固定为 `openai`。
- `base_url`：OpenAI API base URL，默认 `https://api.openai.com/v1`。
- `api_key`：只在创建或覆盖更新时提交，后端加密落库，前端不回显明文。
- `model_name`：后续目标生成和 AI 教练调用的模型名称。
- `api_mode`：第一版默认 `responses`。
- `is_enabled`：是否参与后端 LLM 调用自动选择。
- `is_preferred`：是否作为当前用户首选模型资产。
- `is_active`：是否为后端当前粘性通讯资产。
- `failure_count`：连续失败次数，达到 3 次后后续调用切换到其他启用资产。

API 设置页以列表展示多个 API 资产，新增和编辑使用弹窗。用户没有启用的首选 API 资产时，应先进入 API 设置页；后续目标校准、AI 教练、RAG 总结和代码 review 都通过统一服务选择当前登录用户的 API 资产：优先保持当前通讯资产，连续失败达到 3 次后切换到其他启用且可用的资产。兼容字段 `is_default` 短期保留，语义等同 `is_preferred`。
...
- 目标校准、追问回答和计划草稿生成必须提供 ChatGPT 式流式反馈：用户可以看到可滚动的实时输出区、后台阶段进度和停止生成入口。
- 结构化模型输出只作为后端草稿来源，前端实时输出区不得直接展示原始 JSON、题单 schema 或未校验题目 slug。
- 用户停止生成后，半截输出只能作为过程反馈保留，不能被确认成正式学习计划或训练结果。
- LLM 生成阶段化学习计划草稿，后端用本地题库校验题目 slug、付费状态、重复项和可替换题目。
- 学习计划生成过程中，未经过本地题库校验和 repair 的题单不得作为正式题单展示。
- 目标校准追问和学习计划草稿默认使用简体中文语境；面向用户展示的计划名称、阶段说明、验收标准和推荐理由应为简体中文，`problem_slug`、难度、训练模式、标签 slug、枚举值和代码语言名称等机器字段保留原始英文或枚举值。
- 用户确认草稿后，正式学习计划名称由后端在 LLM 草稿标题基础上追加目标类型、默认训练语言和确认时间；同一用户出现同名计划时，后端追加序号兜底，保证学习计划历史中名称可区分。
...
- 题库列表查询响应时间小于 500ms。
- 单次 AI 回复首包时间目标小于 3s。
- 大模型调用应提供流式反馈，用户应能看到可展示的输出片段和阶段进度。
...
- 用户刷新页面后，可以恢复当前做题进度。
- 用户刷新页面后，应能恢复 LLM Run 状态和已完成的最终结果，不要求回放完整 token。
...
1. 注册或登录本地账号。
2. 保存、测试并设置首选 OpenAI API 资产，且前端不回显 API key 明文。
3. 完成首访目标校准，由大模型生成目标和任务草稿，并在用户确认后生成基础训练计划。
4. 浏览题库或从学习计划进入推荐题目。
5. 打开一道题并看到中文 Markdown 题面。
6. 和 AI 教练围绕这道题进行启发式对话。
7. AI 能判断用户当前卡点。
8. AI 能根据 hint level 控制提示粒度，并用用户可理解的提示档位展示。
9. 用户能在系统内写 Python 代码。
10. AI 能 review 用户代码。
11. 系统能运行基础样例并返回结构化结果。
12. 用户能跳转 LeetCode 提交。
13. 用户能回填提交结果。
14. AI 能基于提交结果进行复盘。
15. 系统能记录用户薄弱点、提示使用情况和题目进度。
16. 系统能展示题型掌握度、常见卡点、平均提示等级和下一组推荐训练。
17. 用户能启动一次轻量面试模拟，并获得结构化评分与改进建议。
18. 系统能展示一次 Agent 执行 trace。
19. 系统能从本地教程语料中检索算法概念或派生教练卡片。
